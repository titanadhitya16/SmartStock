...
public class FrmtransaksiKeluar extends javax.swing.JDialog {
    ...
    private void cetakLaporan(int id_barang_keluar) {
    try {
            HashMap hash = new HashMap();
            hash.put("nm_supplier", cmbid_pelanggan.getSelectedItem().toString());
            hash.put("id_transaksi", id_barang_keluar);
    ...
    String row_tgl = txttgl.getText();
        ...
        Integer id_barang_keluar = 0;
        String id;
        Integer jumlah, stok, not_found, empty = 0;
        ...
        try {
            ...
            // 1. Simpan data ke tabel trbarang_keluar
            stm.executeUpdate("INSERT INTO trbarang_keluar(tgl, id_petugas, id_pelanggan) VALUES ('" + row_tgl + "', '" + row_idpetugas + "', '" + row_idpelanggan + "')");

            // 2. Dapatkan id_barang_keluar yang baru saja dibuat
            java.sql.ResultSet sql = stm.executeQuery("SELECT MAX(id_barang_keluar) as max FROM trbarang_keluar");
            sql.next();
            id_barang_keluar = sql.getInt("max");

            // 3. Simpan data ke tabel trbarang_keluar_detail
            for (int i = 0; i < rowCount; i++) {
                ...
                sql = stm.executeQuery("SELECT stok FROM tmbarang WHERE id_barang = '" + id + "'");
                sql.next();
                if (sql.getRow() == 1){
                ...
                if(not_found == 0){
                    ...
                    stm.executeUpdate("UPDATE tmbarang SET stok='" + stok + "' WHERE id_barang = '" + id + "'");
                    ...
                    JOptionPane.showMessageDialog(null, "Sistem tidak menemukan barang", "Barang Gagal Disimpan", JOptionPane.ERROR_MESSAGE);
                }
            }

            // Opsi jika terdapat barang yang belum satupun di masukan
            if(empty == 0){
                stm.executeUpdate("DELETE FROM trbarang_keluar WHERE id_barang_keluar = '" + id_barang_keluar + "'");
            }
    ...
    private void initComponents() {
        ...
        setTitle("Smartstock :: Transaksi Barang Masuk");
        ...
        if(rowCount > 0 && !"".equals(row_tgl) && !"".equals(row_idpelanggan) && !"".equals(row_idpetugas)){
            ...
            for (int i = 0; i < rowCount; i++) {
                ...
                try {
                    Connection conn = konek.openkoneksi();
                    java.sql.Statement stm = conn.createStatement();
                    java.sql.ResultSet sql = stm.executeQuery("SELECT stok FROM tmbarang WHERE id_barang = '" + id + "'");
    ...
    private void cmbid_pelangganItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cmbid_pelangganItemStateChanged
        ...
        if(!kode.equals("Pilih")){
            try {
                Connection conn = konek.openkoneksi();
                java.sql.Statement stm = conn.createStatement();
                java.sql.ResultSet sql = stm.executeQuery("SELECT id_pelanggan FROM tmpelanggan WHERE kode='"+kode+"'");
                if(sql.next()){
                    txtid_pelanggan.setText(sql.getString("id_pelanggan"));
                }
                konek.closekoneksi();
            } catch (SQLException e) {
                JOptionPane.showMessageDialog(null, "Error " + e);
